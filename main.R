box::use(
  ...
  ./src/deepseek[ask_with_tools, ask_for_args],
  ./src/prompts,
  ./src/tools[
    TOOL_SEARCH, TOOL_VALIDATE_SYMBOLS, TOOL_RECALL_MEMOS, TOOL_SCRAPE_URL,
    TOOL_GET_BARS_MULTI, TOOL_COMPUTE_FEATURES, TOOL_SUBMIT_TRADES,
    TOOL_POST_TWEETS, TOOL_HANDLERS
  ],
...
cycle_result <- tryCatch(
  {
    ...
    wallclock_text <- sprintf(
      paste(
        "It is %s %s ET (%s slot of today's session). Account this when sizing trades —",
        "pre-close is for finalising the day's positioning, not for opening fresh swings",
        "you won't watch."
      ),
      et_weekday,
      format(et_now, "%Y-%m-%d %H:%M"),
      cycle_slot
    )
    ...
    research_response <- ask_with_tools(
      prompt = sprintf(
        paste(
          ...
          paste(
            "3) Draft 2-4 candidate tickers (US-listed, NYSE/NASDAQ/ARCA/AMEX, common stock —",
            "use exact exchange symbols, e.g. BRK.B not BERKSHIRE)."
          ),
          paste(
            "4) Call validate_symbols on your draft. Replace any invalid ones and re-validate",
            "until all are valid."
          ),
          "5) Reply ONLY with JSON of the form:",
          paste(
            '{ "picks": ["TICKER1","TICKER2",...],',
            '"rationale": "one paragraph: why these tickers, what news drives them',
            '(cite scraped sources where used)" }'
          ),
    ...
    decisions_dt <- rbindlist(lapply(decisions, function(d) {
      return(list(symbol = d$ticker, model_action = d$action, model_conf = d$confidence))
    }))
    print(merge(
    ...
    if (hours_since_last < 20) {
      ...
      market_note <- if (is_trading_day) {
        paste(
          "Market was OPEN today — your status tweet can reference today's trades,",
          "fills, and current positions."
        )
      } else {
        paste(
          "Market is CLOSED today (weekend or holiday). No trades were placed.",
          "Status tweet should reflect that — analysis, commentary, what you're",
          "watching for the next trading day, reactions to weekend news.",
          "Don't pretend you traded."
        )
      }
    ...
    list(
      ...
      decisions_summary = paste(
        vapply(
          decisions,
          function(d) {
            return(sprintf("%s:%s", d$ticker, d$action))
          },
          character(1)
        ),
        collapse = ","
